...
- 📋 **表・チェック・引用** — 下のバーは「見出し／文字／リスト／入れる」のまとまりから選ぶ形。
  表の中では行と列の操作がその場に出る。チェックの取り消し線はリストごとに切り替え
- 🎨 **文字の色・下線・マーカー** — 大事なところに色を置ける。色は名前で覚えているので、
  明るい配色でも暗い配色でも読める色になる（外から貼り付けた色も近い色に寄せて受け取る）
- 🚪 **学びのとびら** — 学びたいことのリンク集。別のタブで開いて読み、戻ってきて書く。
  「書くことが無い日」の入口になる（リンクから書き始めたメモには出どころが残る）
- 🔎 **メモの中を探す** — 当たったところが全部光り、↑↓ で行き来。置換は開いたときだけ出る。
  かな・大文字小文字・全角半角は区別しない（一括置換も 1 回で戻せる）
...
- 🔔 **復習のリマインド（選択制）** — 決めた時刻に、その日の復習があるときだけお知らせ。
  端末の中だけで動くので、配信用のサーバーは使わない
